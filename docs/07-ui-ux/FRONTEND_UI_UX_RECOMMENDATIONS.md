...
## ⚠️ Важное обновление (18 марта 2026)

**Экспорт данных: CSV заменён на PDF**

В этом документе упоминается экспорт в CSV. Однако было принято решение реализовать **экспорт в PDF** вместо CSV.

**Причины:**
- PDF удобнее для печати и отчётов
- Сохраняются стили, градиенты и форматирование
- Формат A4, горизонтальная ориентация
- Профессиональный вид для руководства

**Актуальная спецификация:** [../10-future-features/PDF_EXPORT.md](../10-future-features/PDF_EXPORT.md)

**Изменения:**
- `handleExportCSV` → `handleExportPdf`
- `DownloadIcon` → `PictureAsPdfIcon`
- Время реализации: 1 час → ~4 часа (html2pdf.js)
