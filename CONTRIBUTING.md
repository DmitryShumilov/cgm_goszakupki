...
Проект имеет приоритизированный план оптимизации, разделённый на 4 недели:

| Неделя | Фокус | Приоритет |
|--------|-------|-----------|
| **Неделя 1** | Безопасность (CORS, Rate Limiting, Secrets) | 🔴 Высокий |
| **Неделя 2** | Производительность БД (Connection Pool, Индексы) | 🟡 Средний |
| **Неделя 3** | Кэширование и рефакторинг (Redis, модули) | 🟡 Средний |
| **Неделя 4** | Дополнительные оптимизации (тесты, мемоизация) | 🟢 Низкий |
...
2. **Согласуйте** приоритеты с командой
3. **Создайте ветку** для каждой недели:
   ```bash
   git checkout -b feature/optimization-week-1
   ```
4. **Выполните задачи** из чеклиста недели
5. **Протестируйте** изменения
6. **Создайте PR** с описанием изменений
7. **Обновите статус** в `OPTIMIZATION_PLAN.md` после мержа
